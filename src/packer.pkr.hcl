...
variable "build_bucket" {
  default     = ""
  description = "The S3 bucket containing the Cobalt Strike installer."
  type        = string
}
...
source "amazon-ebs" "teamserver" {
  ...
  ami_name                    = "teamserver-hvm-${local.timestamp}-x86_64-ebs"
...
build {
  sources = ["source.amazon-ebs.teamserver"]
